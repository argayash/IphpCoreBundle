...
class RubricRepositoryFunctionalTest extends WebTestCase
{
    ...
    public function testFillTree()
    {
        ...
        $rubric3 = $repo->find(4);
        $this->assertSame($rubric3->getLeft(), 6);
        $this->assertSame($rubric3->getRight(), 7);

        //У рубрики 11111 left=2, right = 3
        $rubric1 = $repo->find(2);
        $this->assertSame($rubric1->getLeft(), 2);
        $this->assertSame($rubric1->getRight(), 3);
       ...
       print $rubric3.',left:'.$rubric3->getLeft();*/
        ...
        $rubric1 = $repo->find(2);
        $this->assertSame($rubric1->getLeft(), 6);
        $this->assertSame($rubric1->getRight(), 7);
    }
}
