...
class BlockAdmin extends Admin
{
    ...
    protected function configureListFields(ListMapper $listMapper)
    {
        $listMapper
                ...
                ->addIdentifier('type')
                ->add('rubric')
                ->add('parent', null, ['label' => 'Parent Block'])
                ->add('position')
    ...
    protected function configureFormFields(FormMapper $formMapper)
    {
       ...
       /* was iphp_block_service_choice, for what??*/
                ->add('type', 'sonata_block_service_choice', ['context' => 'cms'], ['value_strategy' => 1])
                ->add('enabled')
                ->add('parent', 'parentblock_choice', ['label' => 'Parent Block', 'required' => false])
                ->add('rubric', 'rubricchoice', ['required' => false])
                ->add('position');
    ...
    public function preUpdate($object)
    {
        ...
        if ($parent) {
            $object->setRubric($parent->getRubric());
        }
